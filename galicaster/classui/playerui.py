...
import time
import logging
import thread # Change for threading
...
class PlayerClassUI(gtk.Box):
	...
	builder = gtk.Builder()
        builder.add_from_file(path.join(path.dirname(path.abspath(__file__)), 'player.glade'))
...
#------------------------- PLAYER ACTIONS ------------------------
    ...
    def on_play_clicked(self,button):
        ...
        self.thread_id = thread.start_new_thread(self.timer_thread, ()) #TODO timer thread
    ...
    def on_quit_clicked(self,button):
        gui = gtk.Builder()
        gui.add_from_file(path.join(path.dirname(path.abspath(__file__)),"..","ui", "quit.glade"))       
...
#------------------------- PACKAGE ACTIONS ------------------------
	...
	builder = gtk.Builder() # create a module for dialogs
        builder.add_from_file(path.join(path.dirname(path.abspath(__file__)), 'save.glade'))       
    ...
    def ingest(self,package):
        ...
        if not self.network:
            ...
            if package.status in [ mediapackage.RECORDED, mediapackage.PENDING, 
                ...
                self.repository.ingest(package, server['host'], 
                                   server['username'],server['password'], server['workflow'] )
            else:
    ...
    def old_on_question(self,button):
        ...
        if package.status == mediapackage.PENDING:
            gui.add_from_file(path.join(path.dirname(path.abspath(__file__)), 'ingest2.glade'))
        else:
            gui.add_from_file(path.join(path.dirname(path.abspath(__file__)), 'ingest.glade'))
    ...
    def on_question(self,button):
        ...
        if context.get_conf().conf.get("ingest", "active") != "True":			
            text = {"title" : "Media Manager",
