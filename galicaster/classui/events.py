...
class EventManager(gtk.Widget):
    ...
    def __init__(self, number = None, size = None, parent = None): 
        ...
        gui = gtk.Builder()
        gui.add_from_file(path.join(path.dirname(path.abspath(__file__)), 'next.glade'))
        ...
        #title
        #tt = pango.FontDescription("bold "+str(int(k1*200))+"px")
        #title.modify_font(tt)
        ...
        if row == 1:
            pass
            #t = self.big_label("No scheduled events",self.size)
            #t.set_alignment(0.5,0.5)
            #table.attach(t,0,4,row-1,row,gtk.EXPAND|gtk.FILL,True,0,0)
