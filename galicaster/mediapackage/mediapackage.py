...
class Mediapackage():
    ...
    def getElementById(self, identifier, etype=None):
        elem = self.elements.get(identifier)
    ...
    def marshalDublincore(self):
        ...
        if self.metadata_episode.has_key('temporal') and not self.hasTracks():
    ...
    #FIXME merge with add
    def addAttachmentAsString(self, content, name=None, rewrite=False):
        assert path.isdir(self.getURI())
        ...
        if not rewrite:
            self.add(f_path, TYPE_ATTACHMENT)
