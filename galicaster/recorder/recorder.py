...
class Recorder(object):
    ...
    def __on_state_changed(self, bus, message):
        ...
        #print message.src.get_property('name') + "  From: " + str(old) + " to: " + str(new)
        if (isinstance(message.src, gst.Pipeline) and (old, new) == 
    ...
    def __set_vumeter(self, message):
        ...
        # TODO rescaled in vumeter
        if struct['rms'] < -40:  # cut to -40dB
            valor = 0.0
        elif struct['rms'] == float("-inf"):
            valor = 0.0
        else:                
            valor = (struct['rms'][0] + 40) / 40
            if valor < 0:
                valor = 0.0             

        if not self.mute:        
            # WARNING if the gain is to high wont recognize mute properly
            if struct['rms'] == float("-inf") or struct['rms'][0] < -68:
