...
class Scheduler():
    ...
    def __init__(self):
        ...
        self.dispatcher.connect("galicaster-quit", self.do_stop_timers)
        self.__init_client()
        self.t_stop = None
    ...
    def __init_client(self):
        ...
        try:
            self.client = MHHTTPClient(server['host'], server['username'], server['password'])
        except:
    ...
    def set_state(self):
        ...
        try:
            res = self.client.setstate(socket.gethostname(), socket.gethostbyname(socket.gethostname()), self.ca_status)
            res = self.client.setconfiguration(socket.gethostname(), socket.gethostbyname(socket.gethostname())) 
        except:
    ...
    def proccess_ical(self):
        ...
        try:
            ical_data = self.client.ical(socket.gethostname())
        except:
            log.warning('Problems to connect to matterhorn server ' + res)
            self. __stop_timers()
            return
    ...
    def ingest(self):
        ...
        for mp in mps:
            log.info('Ingesting recording start %s', mp.getIdentifier())
            mp.status = mediapackage.INGESTING
            try:
                res = self.client.setrecordingstate(mp.getIdentifier(), "uploading")
            except:
                log.warning('Problems to connect to matterhorn server ')
                self. __stop_timers()
            ...
            self.repo._export_to_zip(mp, ifile)

            code = 0
            try:
                code = self.client.ingest(ifile.name, "full", mp.getIdentifier())
            except:
                log.warning('Problems to connect to matterhorn server ')
                self. __stop_timers()
            ...
            ifile.close()
            if code == 200:
                log.info('Ingest recording finished %s with %s', mp.getIdentifier(), code)
                mp.status = mediapackage.INGESTED
                try:
                    res = self.client.setrecordingstate(mp.getIdentifier(), "upload_finishedad")
                except:
                    pass
            else:
                log.info('Failed Ingest recording, stop %s with %s', mp.getIdentifier(), code)
                mp.status = mediapackage.INGEST_FAILED
                try:
                    res = self.client.setrecordingstate(mp.getIdentifier(), "upload_error")
                except:
                    pass
            self.repo.update(mp)
                # TODO Retry
